...
public class TurboBoyerMoore {
    ...
    /**
     *
     * Данный метод ищет вхождения заданного при компиляции паттерна в буфере
     *
     ...
     * @return          Список положительных смещений вхождения паттерна в буфер
     *                  P.S.> Если ищем все вхождения, то в результат вставляется завершающее смещение
     *                  по которому можно взять суффикс буфера и скрепить его с префиксом следующего буфера
     *                  для поиска в несколько итераций с учётом разрыва искомого паттерна границей буферов.
